...
class UI(QMainWindow):
    ...
    def __init__(self):
        ...
        self.file_path = "/home/fyp2selfdriving/Documents/traffic_light/Yolov5/datasets/dualcam/images/train"
        self.save_path = ""        
        self.index = 1
    ...
    def view_img(self):
        fnamenarrow = self.file_path + "/narrow_2_" + self.convert_to_str(self.index) + ".jpg"
        fnamewide = self.file_path + "/wide_2_" + self.convert_to_str(self.index) + ".jpg"
        ...
        self.labelnarrowtext.setText("narrow_2_" + self.convert_to_str(self.index) + ".jpg")
        self.labelwidetext.setText("wide_2_" + self.convert_to_str(self.index) + ".jpg")
        ...
        if with_bbox:
            for ann in image_data.annotations:
                ...
                if (ann.xmax - ann.xmin)<20:
                    fontft.putText(img=image, text=ann.name, org=(ann.xmin-20, ann.ymin-10), fontHeight=15, color=(255, 125, 99), thickness=-1, line_type=cv2.LINE_AA, bottomLeftOrigin=True)
                else:
                    if ann.name == 'Count-down' or ann.name == 'Empty-count-down':
                        fontft.putText(img=image, text=ann.name, org=(ann.xmin-20, ann.ymax+15), fontHeight=40, color=(255, 125, 99), thickness=-1, line_type=cv2.LINE_AA, bottomLeftOrigin=True)
                    else:
                        fontft.putText(img=image, text=ann.name, org=(ann.xmin-20, ann.ymin-10), fontHeight=40, color=(255, 125, 99), thickness=-1, line_type=cv2.LINE_AA, bottomLeftOrigin=True)
